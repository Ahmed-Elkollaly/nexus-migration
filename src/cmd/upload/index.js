...
const upload = async ({
    ...
    function createUploadTask(asset, outputDir) {
        return async () => {
            ...
            repositoryMapper = {
                repository : {
                    "groupId" : "org.maven.com",
                    "artifactId" : "junit",
                    "version" : "1.3"
                } 
            }
            const test = {
                "maven2.groupId" : "org.maven.com",
                "maven2.artifactId" : "junit",
                "maven2.version" : "1.3"
            }
            console.debug(`⏱ Upload ${repository}/${realFilePath}`);
            console.log("Mapper: ",repositoryMapper )
            console.log("Mapper Repo: ",repositoryMapper[repository])
            const uploadUrl = `${nexusUrl}/service/rest/v1/components?repository=maven-releases`;
